...
// 게시글 추가하기(유저정보는 리덕스에 있으니 컨텐츠만 가져올것)
const addPostFB = (contents = "") => {
  return function (dispatch, getState, { history }) {
    ...
    const _upload = storage
      .ref(`images/${user_info.user_id}_${new Date().getTime()}`) //이미지 이름이지만 중복되지 않게 날짜+유저정보로 파일이름을 지정해줌 
      .putString(_image, "data_url");

    _upload.then((snapshot) => {
      snapshot.ref
        .getDownloadURL()
        .then((url) => { //파이어 스토리지에 저장한 url 획득, 이제 addpost에 실어주자
          return url;
        })
        .then((url) => {
          ...
          postDB
            ...
            .then((doc) => {
              ...
              // 프리뷰는 이제 null로 바꿔줍니다! 왜? 다음번에 글쓰러 갔을때 그대로 있을테니깐(프리뷰는 리덕스에 있으니깐)
              dispatch(imageActions.setPreview(null));
            })
            .catch((err) => {
